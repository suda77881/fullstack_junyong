## Track001 -  github
   ...
   5. flutter + spring boot + jpa + mybatis + thymeleaf + react
...
발생문제 : 로컬 master 와 원격브랜치 (origin/master) 차이발생
해결방안 : incorect와 current 간에 내용차이로 발생한 문제로 두 파일을 한 쪽을 선택하여 merge하거나, result에 둘의 내용을 참고하여 수동 merge가 가능하다.
          이후 `git add` > git commit 커맨드를 실행하여 
