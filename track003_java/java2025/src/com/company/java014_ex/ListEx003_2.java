...
public class ListEx003_2 {
    public static void main(String[] args) {
    	//## 힌트2) List로   IceCreamDTO2 자료형만 입력받을수 있는 동적배열 만들기
        ...
        while(true) {
            ...
            switch(choice) {
            	case 1 :  //1. 추가 아이스트림 이름, 가격입력받아서 list에 추가 list.add(  아이스크림객체 )
            		System.out.println("아이스크림 이름 : "); String name = scanner.next();
            		System.out.println("아이스크림 가격 : "); int price = scanner.nextInt();
            	...
            	case 2 :  //2. 목록 아이스크림목록  list
            		...
            		//※ list.isEmpty() 비었는지 아니었는지 true false
            		for (IceCreamDTO2 ice : list) {
            			System.out.println("- " + ice);  // toString 
            		}
            	break;
            	case 3 :  //3. 제거 아이스크림 이름입력받아서 제거   list.remove()
            		System.out.println("제거할 아이스크림 이름 : "); String rname = scanner.next();
            		int find = -1;
            		for(int i=0; i<list.size(); i++) {
            			// 		 해당번호의 이름       .같니(   입력한이름) 
            			if ( list.get(1).getName().equals(rname)) {find=i; break;}
            		}
            		list.remove(find);
            		System.out.println("★" + rname + "삭제완료 !");
            	break;/////////////////////////////////////////////////////////////
            	case 4 :  //4. 검색  내가 입력한 아이스크림이 있는지없는지 검색   contains  
            		System.out.println("검색할 아이스크림 이름"); String fname = scanner.next();
...
2. 클래스 구조
클래스명	역할	주요 메서드
IceCreamDTO2	아이스크림 정보 저장 DTO   	/ getName(), getPrice(), toString()
ListEx003   메인 실행 클래스	main(), List 활용

- IceCreamDTO2는 이름과 가격을 저장하는 데이터 객체
- ListEx003                클래스는 List<IceCreamDTO2>를 활용하여 아이스크림을 추가, 출력, 제거, 검색
- add, get, size, remove, contains 메서드를 모두 활용

3. 요구사항
- IceCreamDTO2 클래스를 정의하고, 
이름과 가격을 저장할 수 있도록 생성자 및 getter 작성
- ListEx003 클래스에서 List<IceCreamDTO2>를 생성하고, add() 메서드로 아이스크림 추가
...
ArrayList<IceCreamDTO2>를 사용하여 아이스크림 목록을 저장할 것
