...
public class Network002_Server {
	public static void main(String[] args) {
		//#1. 서버소켓 ( AS 센터 ) [socket,socket,socket,,,]
		...
		try {
			...
			// 3-1. 연결요청을 기다리는 중인데 ,,,, 연결이 오면 수락할께
			// 3-2. 연결이 들어올떄까지 STOP
			// 3-3. 연결이 들어오면 .... Socket으로 연결
			System.out.println("[Server] 4. 고객연결 들어와서 상담사랑(socket)연결함... ");
...
////////////////////////////////////////////////////////////
//		InputStream(소버소켓) > 프로그램 기준 > OutputStream(클라이언트)
// 말하기 기능 Sender 듣기 기능  Receiver
//////////////////////////////////////////////////
...
class Sender	extends Thread{	
	...
	@Override public void run() {
		...
		try {
			...
			try {
				if( out!= null ){ out.close();	}	// 연결끊기
				if( br != null ){ br.close();		}
				if(!socket.isClosed() ) {	socket.close();	}	// 소켓끊기			
			}catch(IOException e) {e.printStackTrace();}
		}
...
/****
 ...
 2. Socket 	통신 - 양방향통신 (특정포트를 통해 실시간으로 정보주고받기 - tcp)
 3. 소켓통신  흐름 
 	1) 서버소켓 (as 센터), 포트바인딩(문열놓기)
 	2) 클라이언트 연결 요청 , 수락 
 	3) 클라이언트 소켓(socket) 상담사가 읽어들이기 ( InputStream(소버소켓) > 프로그램 기준 > OutputStream(클라이언트) )		
 												상담사가 읽어들이기                    고객에게 말하기
    4) 말하고 주고받기 
